  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="card mt-3">
          ...
          <div class="card-body">
            <a href="{{ route('pelayan.menutambah') }}" class="btn btn-icon icon-left btn-danger mb-3 px-3"><i class="fas fa-plus"></i></a>
            @if(session('message'))
                  ...
                  <tbody class="mt-2">
                    @foreach ($data_masakan as $no => $data)
                      <tr>
                          ...
                          <td></td>
                          <td>
                            <a href="{{ route('pelayan.menuedit',$data->id_masakan) }}" class="btn btn-success"><i class="fas fa-edit"></i></a>
                            <a href="#" data-id="" class="btn btn-danger confirm_script-{{$data->id_masakan}} mr-3">
                              <form action="{{ route('pelayan.masakandelete',$data->id_masakan)}}" class="delete_form-{{$data->id_masakan}}" method="POST">
                              @method('DELETE')
                              @csrf
                              </form>
                              <i class="fas fa-trash"></i>
                              </a>
                              <form action="{{route('pelayan.masakanupdatestatus',$data->id_masakan)}}" method="post">
                                @csrf
                                @if ($data->status == 'tersedia')
